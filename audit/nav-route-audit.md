...
Summary: {"total":63,"pass":15,"fail":0,"hidden":48,"group":0}

| role | section | parent | title | route | navFlag | navEnt | routeFlag | routeEnt | status | notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| admin | Admin |  | Admin Dashboard | /user/dashboard-admin | ADMIN_CONSOLE | admin.console | ADMIN_CONSOLE | admin.console | PASS |  |
| admin | Operations |  | Requests | /user/requests |  | admin.requests |  | admin.requests | PASS |  |
...
| admin | Billing |  | Plans | /user/admin/plans | SUBSCRIPTIONS | admin.billing.plans | SUBSCRIPTIONS | admin.billing.plans | PASS |  |
...
| admin | Content |  | Feature Flags | /user/admin/feature-flags | ADMIN_CONSOLE | admin.featureflags | ADMIN_CONSOLE | admin.featureflags | PASS |  |
| admin | Content |  | Resume Templates | /user/admin/resume-templates | ADMIN_CONSOLE | admin.resume.templates | ADMIN_CONSOLE | admin.resume.templates | PASS |  |
...
| user |  |  | Dashboard | /user/dashboard |  | user.dashboard |  | user.dashboard | PASS |  |
...
| user | Resume Portal |  | My Resumes | /user/resumes | RESUME_PORTAL | resume.portal | RESUME_PORTAL | resume.portal | PASS |  |
...
| user | Job Tracking |  | Applications | /user/job-applications | JOB_TRACKING | job.tracking | JOB_TRACKING | job.tracking | PASS |  |
...
| user | Job Tracking |  | Job Alerts | /notifications | NAV_PLACEHOLDER | job.alerts | ALERTS | job.alerts | HIDDEN (route exists) | Nav hidden via NAV_PLACEHOLDER; route still present |
| user | Job Tracking |  | Analytics | /user/job-analytics |  | job.analytics |  | job.analytics | PASS |  |
...
| user | Learn |  | Course Central | /user/user-learn | COURSE_CENTRAL | learn.portal | COURSE_CENTRAL | learn.portal | PASS |  |
| user | Learn |  | Saved | /user/learn-saved |  | learn.saved |  | learn.saved | PASS |  |
...
| user |  |  | Notifications | /notifications | ALERTS | job.alerts | ALERTS | job.alerts | PASS |  |
...
| user | Billing |  | Upgrade | /user/billing/upgrade |  | billing.upgrade |  | billing.upgrade | PASS |  |
