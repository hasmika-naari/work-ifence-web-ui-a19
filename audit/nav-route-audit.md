...
Summary: {"total":64,"pass":6,"fail":0,"hidden":58,"group":0}

| role | section | parent | title | route | navFlag | navEnt | routeFlag | routeEnt | status | notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| admin | Admin |  | Admin Dashboard | /user/dashboard-admin | NAV_PLACEHOLDER | ADMIN_CONSOLE | ADMIN_CONSOLE | ADMIN_CONSOLE | HIDDEN (route exists) | Nav hidden via NAV_PLACEHOLDER; route still present |
| admin | Operations |  | Requests | /user/requests | NAV_PLACEHOLDER | ADMIN_REQUESTS |  | ADMIN_REQUESTS | HIDDEN (route exists) | Nav hidden via NAV_PLACEHOLDER; route still present |
...
| admin | Billing |  | Plans | /user/admin/plans | NAV_PLACEHOLDER | ADMIN_BILLING_PLANS | SUBSCRIPTIONS | ADMIN_BILLING_PLANS | HIDDEN (route exists) | Nav hidden via NAV_PLACEHOLDER; route still present |
| admin | Billing |  | User Subscriptions | /user/admin/subscriptions | NAV_PLACEHOLDER | ADMIN_BILLING_SUBSCRIPTIONS | SUBSCRIPTIONS | ADMIN_BILLING_SUBSCRIPTIONS | HIDDEN (route exists) | Nav hidden via NAV_PLACEHOLDER; route still present |
| admin | Billing |  | Upgrade Requests | /user/admin/upgrade-requests | NAV_PLACEHOLDER | ADMIN_BILLING_SUBSCRIPTIONS | SUBSCRIPTIONS | ADMIN_BILLING_SUBSCRIPTIONS | HIDDEN (route exists) | Nav hidden via NAV_PLACEHOLDER; route still present |
...
| admin | Content |  | Feature Flags | /user/admin/feature-flags | NAV_PLACEHOLDER | ADMIN_FEATUREFLAGS | ADMIN_CONSOLE | ADMIN_FEATUREFLAGS | HIDDEN (route exists) | Nav hidden via NAV_PLACEHOLDER; route still present |
...
| admin | Content |  | Resume Templates | /user/admin/resume-templates | NAV_PLACEHOLDER | ADMIN_RESUME_TEMPLATES | ADMIN_CONSOLE | ADMIN_RESUME_TEMPLATES | HIDDEN (route exists) | Nav hidden via NAV_PLACEHOLDER; route still present |
...
| user |  |  | Dashboard | /user/dashboard |  | USER_DASHBOARD |  | USER_DASHBOARD | PASS |  |
...
| user | Resume Portal |  | My Resumes | /user/resumes | NAV_PLACEHOLDER | RESUME_PORTAL | RESUME_PORTAL | RESUME_PORTAL | HIDDEN (route exists) | Nav hidden via NAV_PLACEHOLDER; route still present |
...
| user | Job Tracking |  | Applications | /user/job-applications | NAV_PLACEHOLDER | JOB_TRACKING | JOB_TRACKING | JOB_TRACKING | HIDDEN (route exists) | Nav hidden via NAV_PLACEHOLDER; route still present |
...
| user | Job Tracking |  | Analytics | /user/job-analytics |  | JOB_ANALYTICS |  | JOB_ANALYTICS | PASS |  |
...
| user | Learn |  | Course Central | /user/user-learn | NAV_PLACEHOLDER | LEARN_PORTAL | COURSE_CENTRAL | LEARN_PORTAL | HIDDEN (route exists) | Nav hidden via NAV_PLACEHOLDER; route still present |
| user | Learn |  | Saved | /user/learn-saved | NAV_PLACEHOLDER | LEARN_SAVED |  | LEARN_SAVED | HIDDEN (route exists) | Nav hidden via NAV_PLACEHOLDER; route still present |
...
| user |  |  | Notifications | /notifications | ALERTS | JOB_ALERTS | ALERTS | JOB_ALERTS | PASS |  |
...
| user | Billing |  | Upgrade | /user/billing/upgrade |  | BILLING_UPGRADE |  | BILLING_UPGRADE | PASS |  |
...
| user |  |  | My Profile | /user/profile |  | PROFILE |  | PROFILE | PASS |  |
