...
getHTMLTemplateForMonoPro(resumeForm: Resume) {
  ...
  <body style="font-family:'Poppins',Arial,Helvetica,sans-serif; color:#000; margin:0; padding:20px;">

    <!-- Header Section -->
    <div style="margin-bottom:18px;">
      <!-- Name and Title -->
      <div>
        <p style="font-size:28px; font-weight:600; margin:0; text-transform:uppercase;">${resumeForm.contact.fname} ${resumeForm.contact.lname}</p>
        ${resumeForm.contact.subTitle?.length > 0 ? 
          `<p style="font-size:16px; font-weight:400; color:#333; margin:2px 0 6px 0;">${resumeForm.contact.subTitle}</p>` 
        : ''}
        <div style="height:3px; background-color:#39A5B7; width:100%; margin:4px 0 8px 0;"></div>
      </div>

      <!-- Contact Line -->
      <div style="font-size:11px; color:#000; white-space:nowrap; margin-top:4px;">
        ${
          [
            resumeForm.contact.phone_number ? `<span style="margin-right:6px;">📞 ${resumeForm.contact.phone_number}</span>` : '',
            resumeForm.contact.email ? `<span style="margin-right:6px;">✉️ ${resumeForm.contact.email}</span>` : '',
            resumeForm.contact.linkedIn_profile ? `<a href="${resumeForm.contact.linkedIn_profile}" style="text-decoration:none; color:#39A5B7; margin-right:6px;">LinkedIn</a>` : '',
            resumeForm.contact.github_profile ? `<a href="${resumeForm.contact.github_profile}" style="text-decoration:none; color:#39A5B7;">GitHub</a>` : ''
...
getFormattedSectionsForMonoPro(resume: Resume) {
  ...
  // Common style snippets for reuse
  const titleStyle = `
    font-weight:600;
    color:#384347;
    text-transform:uppercase;
    border-bottom:1px solid rgba(101,105,109,0.5);
    font-size:14px;
    display:block;
    margin:12px 0 6px 0;
    padding-bottom:2px;
  ...
  return resume.sections.map((e: SectionDesc) => {
    const title = `<span style="${titleStyle}">${e.editable_section_title}</span>`;
    ...
    switch (e.section) {
      ...
      // --- Skills (category split into two columns) ---
      case 'SKILLS_CATEGORY':
        return resume.skill_v2?.length > 0
          ? `
            <section style="${sectionContainerStyle}">
      ...
      // --- Accomplishment with description ---
      case 'ACHIEVEMENT_WITH_DESC':
        return resume.accomplishment?.length > 0
          ? `
            <section style="${sectionContainerStyle}">
...
public getEducationSectionForMonoPro(items: Education[]): string {
  return items.map((item: Education) => `
    <table style="width:100%; border-collapse:collapse; margin-top:6px; font-family:'Poppins',Arial,Helvetica,sans-serif; font-size:8px; color:#000;">
      <tr>
        <td style="width:70%; text-align:left; vertical-align:top; padding:0;">
          ${item.degree?.length > 0 && item.field_of_study?.length > 0 ? 
            `<p style="margin:0; padding:0;"><b>${item.degree}, ${item.field_of_study}</b></p>` :
          item.degree?.length > 0 ? 
            `<p style="margin:0; padding:0;"><b>${item.degree}</b></p>` :
          item.field_of_study?.length > 0 ? 
            `<p style="margin:0; padding:0;"><b>${item.field_of_study}</b></p>` : ''}
        </td>

        <td style="width:30%; text-align:right; vertical-align:top; padding:0;">
          ${item.school_location?.length > 0 ? 
            `<p style="margin:0; padding:0;">${item.school_location}</p>` : ''}
        </td>
      </tr>
      ...
      <tr>
        <td style="width:70%; text-align:left; vertical-align:top; padding:0;">
          ${item.school_name?.length > 0 ? 
            `<p style="margin:0; padding:0;">${item.school_name}</p>` : ''}
        </td>
        <td style="width:30%; text-align:right; vertical-align:top; padding:0;">
          ${item.graduation_date?.length > 0 ? 
            `<p style="margin:0; padding:0;">${item.graduation_date}</p>` : ''}
        </td>
      </tr>
      ...
      <tr>
        <td colspan="2" style="text-align:left; padding:0;">
          <p style="margin:0; padding:0;">GPA: <b>${item.gpa}</b></p>
        </td>
      </tr>` : ''}
    </table>
  `).join('');
...
public getSkillsWithBulletPointsSectionForMonoPro(items: Skill[]): string {
  return items
    .map((item: Skill) => {
      const skillText = item?.name || (item as unknown as string);
      return skillText && skillText.trim().length > 0
        ? `
          <li style="
            margin: 2px 0;
            padding: 0;
  ...
  return items
    .map(
      ...
      <li style="
        ...
        text-align: left;
      ">
        <span style="font-weight:600;">${item.sub_title}:</span>
        <span style="font-weight:400;"> ${item.skills.join(', ')}</span>
      </li>
    `
    )
    .join('');
...
public getExperienceSectionForMonoPro(items: Experience[]): string {
  return items
    .map(
      (item: Experience) => `
      <table style="width:100%; border-collapse:collapse; margin-top:8px; font-family:'Poppins', Arial, Helvetica, sans-serif; color:#000; font-size:8px;">
        <!-- Header row: Job title and date -->
        <tr>
          <td style="width:70%; text-align:left; vertical-align:top; padding:0;">
            <p style="margin:0; padding:0;">
              <b>${item.position_title}</b>${item.company_name ? `, ${item.company_name}` : ''}
            </p>
          </td>
          <td style="width:30%; text-align:right; vertical-align:top; padding:0;">
            ${
              item.start_date?.length > 0
                ? `<p style="margin:0; padding:0;">${item.start_date}${
                    item.end_date?.length > 0 ? ` - ${item.end_date}` : ''
                  }</p>`
                : ''
            }
          </td>
        ...
        <tr>
          <td colspan="2" style="padding-top:2px;">
            <div style="margin-top:3px; font-size:8px; line-height:1.5;">
              ${item.description
                ?.replace(
                  /<ul>|<\/ul>|<li>|<\/li>/g,
                  (match) => {
                    switch (match) {
                      case '<ul>':
                        return `<ul style="margin:2px 0 0 14px; padding:0; list-style-type:disc; list-style-position:outside;">`;
                      case '<li>':
                        return `<li style="margin:2px 0; padding:0; font-family:'Poppins', Arial, Helvetica, sans-serif; color:#000;">`;
                      case '</li>':
                        return '</li>';
                      case '</ul>':
                        return '</ul>';
                ...
                .replace(
                  ...
                  (match) =>
                    match === '<p>'
                      ? `<p style="margin:0; padding:0; font-size:8px; color:#000; line-height:1.5;">`
                      : '</p>'
                )}
            </div>
          </td>
      ...
      <table style="width:100%; border-collapse:collapse; margin-top:8px; font-family:'Poppins', Arial, Helvetica, sans-serif; color:#000; font-size:8px;">
        ...
        <tr>
          <td style="width:70%; text-align:left; vertical-align:top; padding:0;">
            ${
              item.project_name?.length > 0
                ? item.project_link?.length > 0
                  ? `<p style="margin:0; padding:0;">
                      <a href="${item.project_link}" style="color:#000000DE; text-decoration:underline;">
...
public getCertificationSectionForMonoPro(items: Certification[]): string {
  return items
    .map((item: Certification) => `
      <table style="width:100%; border-collapse:collapse; margin-top:6px; font-family:'Poppins', Arial, Helvetica, sans-serif; color:#000; font-size:8px;">
        <tr>
          <td style="width:70%; text-align:left; vertical-align:top; padding:0;">
            ${
              item.certification_link && item.certification_link.length > 0
                ? `<p style="margin:0; padding:0;">
                    <a href="${item.certification_link}" style="color:#000000DE; text-decoration:underline;"><b>${item.certification_name}</b></a>
                    ${item.issued_organisation ? `<span style="margin-left:6px;">, ${item.issued_organisation}</span>` : ''}
                  </p>`
                : `<p style="margin:0; padding:0;"><b>${item.certification_name}</b>${item.issued_organisation ? `<span style="margin-left:6px;">, ${item.issued_organisation}</span>` : ''}</p>`
            }
          </td>
          <td style="width:30%; text-align:right; vertical-align:top; padding:0;">
            ${
              (item.issued_month && item.issued_month.length > 0) || (item.issued_year && item.issued_year.length > 0)
                ? `<p style="margin:0; padding:0;">${item.issued_month ? item.issued_month + ' ' : ''}${item.issued_year ? item.issued_year : ''}</p>`
                : ''
            }
          </td>
        </tr>
  ...
  return items
    .map(
      ...
      <table style="width:100%; border-collapse:collapse; margin-top:8px; font-family:'Poppins', Arial, Helvetica, sans-serif; color:#000; font-size:8px;">
        ...
        <tr>
          <td style="width:70%; text-align:left; vertical-align:top; padding:0;">
            <p style="margin:0; padding:0;"><b>${item.accomplisment}</b></p>
          </td>
          <td style="width:30%; text-align:right; vertical-align:top; padding:0;">
            ${
              item.date?.length > 0
                ? `<p style="margin:0; padding:0;">${item.date}</p>`
                : ''
            }
          </td>
        </tr>
...
getHTMLTemplateForModern(resumeForm: Resume) {
  ...
  <body style="font-family:'Cambria', serif; color:#000; margin:0; padding:0; opacity:0.9;">
    <div style="width:100%; margin:0 auto;">

      <!-- Header -->
      <div style="display:block; margin-bottom:12px;">
        <div style="font-family:'Cambria', serif; font-size:17.5px; font-weight:700; border-bottom:2px solid #39A5B7; padding-bottom:2px; margin-bottom:5px;">
          ${resumeForm.contact.fname + ' ' + resumeForm.contact.lname}
        </div>

        <div style="font-size:11.5px; color:#000; display:flex; flex-wrap:wrap; gap:4px; line-height:1.6;">
          ${resumeForm.contact.address?.length > 0 ? `<span>${resumeForm.contact.address}</span>` : ''}
          ${resumeForm.contact.phone_number?.length > 0 && resumeForm.contact.address?.length > 0 ? `<span> | </span>` : ''}
          ${resumeForm.contact.phone_number?.length > 0 ? `<span>${resumeForm.contact.phone_number}</span>` : ''}
          ${resumeForm.contact.email?.length > 0 ? `<span> | </span><span>${resumeForm.contact.email}</span>` : ''}
          ${resumeForm.contact.linkedIn_profile?.length > 0 ? `<span> | </span><a href="${resumeForm.contact.linkedIn_profile}" style="color:#39A5B7; text-decoration:none;">${resumeForm.contact.linkedIn_profile_display_name}</a>` : ''}
          ${resumeForm.contact.github_profile?.length > 0 ? `<span> | </span><a href="${resumeForm.contact.github_profile}" style="color:#39A5B7; text-decoration:none;">${resumeForm.contact.github_profile_display_name}</a>` : ''}
        </div>
      </div>
            ...
            return resume.profileSummary?.profile_summary?.length > 0
              ? `
                <div style="margin-top:8px;">
                  ${title}
                  <div style="font-size:12.5px; color:#000; font-family:'Cambria', serif;">
                  ${
                    resume.profileSummary.profile_summary
                      ?.replace(/<ul>|<\/ul>|<li>|<\/li>/g, (match) => {
                        switch (match) {
                          case "<ul>":
                            return `<ul style="margin:2px 0 0 14px; padding:0; list-style-type:disc; list-style-position:outside;">`;
                          case "<li>":
                            return `<li style="margin:2px 0; padding:0; font-family:'Cambria', serif; color:#000;">`;
                          case "</li>":
                            return "</li>";
                          case "</ul>":
                            return "</ul>";
                      ...
                      .replace(/<p>|<\/p>/g, (match) =>
                        match === "<p>"
                          ? `<p style="margin:0; padding:0; font-size:12.5px; color:#000; line-height:1.5;">`
                          : "</p>"
                      ) || ""
                  }
                  </div>
          ...
          case "RELEVANT_COURSEWORK":
            return resume.courseWork?.length > 0
              ? `
                <div style="margin-top:8px;">
                  ${title}
                  <ul style="margin:4px 0 0 0px; padding:0; list-style-type:disc; list-style-position:outside; font-size:12.5px; line-height:1.4;">
                    ${this.getCourseWorkWithBulletPointsSectionForModern(resume.courseWork)}
                  </ul>
                </div>`
              : "";
          ...
          case "CERTIFICATIONS":
            return resume.certification?.length > 0
              ? `
                <div style="margin-top:8px;">
                  ${title}
                  ${this.getCertificationSectionForModern(resume.certification)}
                </div>`
              : "";
          ...
          case "ACHIEVEMENTS_BULLET_POINTS":
            return resume.achievementBulletPoints?.ach?.length > 0
              ? `
                <div style="margin-top:8px;">
                  ${title}
                  <div style="font-size:12.5px; line-height:1.5; color:#000;">
                    ${resume.achievementBulletPoints.ach}
                  </div>
                </div>`
              : "";
...
public getEducationSectionForModern(items: Education[]): string {
  return items
    .map((item: Education) => {
      // guard null/undefined fields
      const degree = item?.degree || '';
      const field = item?.field_of_study ? `, ${item.field_of_study}` : '';
      const school = item?.school_name || '';
      const location = item?.school_location || '';
      const grad = item?.graduation_date || '';
      const gpa = item?.gpa ? `${item.gpa} GPA` : '';

      return `
        <table style="width:100%; border-collapse:collapse; margin-top:6px; font-family:'Cambria', serif; color:#000; font-size:12.5px;">
          <tr>
            <td style="width:65%; padding:0; vertical-align:top; text-align:left;">
              ${degree || field ? `<p style="margin:0; padding:0;"><b>${degree}${field}</b></p>` : ''}
            </td>
            <td style="width:35%; padding:0; vertical-align:top; text-align:right;">
              ${location ? `<p style="margin:0; padding:0;">${location}</p>` : ''}
            </td>
          </tr>

          <tr>
            <td style="width:65%; padding:0; vertical-align:top; text-align:left;">
              ${school ? `<p style="margin:0; padding:0;">${school}</p>` : ''}
            </td>
            <td style="width:35%; padding:0; vertical-align:top; text-align:right;">
              ${grad ? `<p style="margin:0; padding:0;">${grad}</p>` : ''}
            </td>
          </tr>
          ...
          ${gpa ? `
            <tr>
              <td colspan="2" style="padding:0; vertical-align:top; text-align:left;">
                <p style="margin:0; padding:0;">GPA: <b>${gpa}</b></p>
              </td>
            </tr>
          ` : ''}
        </table>
...
public getSkillsCategorySectionForModern(items: SkillV2[]): string {
  return items
    .map(
      (item: SkillV2) => `
        <li style="
          ...
          font-family:'Cambria', serif;
          list-style-type:disc;">
          <b style="font-weight:700;">${item.sub_title}:</b> 
          ${item.skills.join(', ')}
        </li>
      `
    )
...
public getExperienceSectionForModern(items: Experience[]): string {
  return items
    .map(
      (item: Experience) => `
        <table style="width:100%; border-collapse:collapse; margin-top:8px; font-family:'Cambria', serif; color:#000; font-size:12.5px;">
          <!-- Header Row -->
          <tr>
            <td style="width:70%; text-align:left; vertical-align:top; padding:0;">
              <p style="margin:0; padding:0;">
                <b>${item.position_title}</b> | ${item.company_name}
              </p>
            </td>
            <td style="width:30%; text-align:right; vertical-align:top; padding:0;">
              <p style="margin:0; padding:0;">${item.start_date} - ${item.end_date}</p>
            </td>
          </tr>
          ...
          <tr>
            <td colspan="2" style="padding-top:3px;">
              <div style="font-size:12.5px; line-height:1.5; color:#000; margin:0;">
                ${
                  item.description
                    ?.replace(/<ul>|<\/ul>|<li>|<\/li>/g, (match) => {
                      switch (match) {
...
public getProjectSectionForModern(items: Project[]): string {
  return items
    .map((item: Project) => {
      const projectName = item?.project_name || '';
      const period = item?.period || '';
      const techs = item?.technologies_used || '';
      ...
      return `
        <table style="width:100%; border-collapse:collapse; margin-top:8px; font-family:'Cambria', serif; color:#000; font-size:12.5px;">
          <tr>
            <!-- Project title (left) -->
            <td style="width:100%; padding:0; vertical-align:top; text-align:left;">
              ${
                projectName.length > 0
                  ? `<p style="margin:0; padding:0;"><b>${projectName}</b> ${techs?.length > 0 ? `<span> | </span><span>${techs}</span>` : ''} ${period?.length > 0 ? `<span> | </span><span>${period}</span>` : ''}</p>`
                  : ''
              }
            </td>

            <!-- Period / Technologies (right) -->
          </tr>

          <!-- Description row: sanitize inner HTML lists and paragraphs to have inline styles -->
          <tr>
            <td colspan="2" style="padding-top:4px;">
              <div style="font-size:12.5px; line-height:1.5; color:#000; margin:0;">
                ${
                  description
                    .replace(/<ul>|<\/ul>|<ol>|<\/ol>|<li>|<\/li>/g, (match) => {
                      switch (match) {
                        case '<ul>':
                          return `<ul style="margin:2px 0 0 14px; padding:0; list-style-type:disc; list-style-position:outside;">`;
                        case '<ol>':
                          return `<ol style="margin:2px 0 0 18px; padding:0; list-style-position:outside;">`;
                        case '<li>':
                          return `<li style="margin:2px 0; padding:0; font-family:'Cambria', serif; color:#000;">`;
                        case '</li>':
                          return '</li>';
...
public getAccomplishmentsSectionForModern(items: Accomplishment[]): string {
  return items
    .map(
      (item: Accomplishment) => `
        <table style="width:100%; border-collapse:collapse; margin-top:8px; font-family:'Cambria', serif; color:#000; font-size:12.5px;">
          <!-- Header Row -->
          <tr>
            <td style="width:70%; padding:0; vertical-align:top; text-align:left;">
              ${
                item.accomplisment
                  ? `<p style="margin:0; padding:0; font-weight:700;">${item.accomplisment}</p>`
                  : ''
              }
            </td>
            <td style="width:30%; padding:0; vertical-align:top; text-align:right;">
              ${
                item.date
                  ? `<p style="margin:0; padding:0; color:#000;">${item.date}</p>`
                  : ''
              }
            </td>
          </tr>
...
public getSkillsWithBulletPointsSectionForModern(items: Skill[]): string {
  return items
    .map((item: Skill, index: number) => {
      const skillName = item?.name || (item as unknown as string) || '';

      return `
        <span style="font-size:12.5px; font-family:'Cambria', serif; color:#000;">
          ${skillName}
        </span>
        ${index !== items.length - 1
          ? `<span style="font-size:14px; margin:0 6px; color:#000;">•</span>`
          : ''}
      `;
    })
    .join('');
...
public getCertificationSectionForModern(items: Certification[]): string {
  return items
    .map((item: Certification) => `
      <table style="width:100%; border-collapse:collapse; margin-top:6px; font-family:'Cambria', serif; color:#000; font-size:12.5px;">
        <tr>
          <!-- Left: Certification Name & Issuer -->
          <td style="width:70%; text-align:left; vertical-align:top; padding:0;">
            ${
                  ...
                  <p style="margin:0; padding:0;">
                    ${
                      item.certification_link
                        ? `<a href="${item.certification_link}" target="_blank" style="color:#000; text-decoration:underline;">${item.certification_name}</a>`
                        : `<span style="color:#000;">${item.certification_name}</span>`
                    }
                    ${item.issued_organisation ? `, ${item.issued_organisation}` : ''}
                  </p>
                `
                : ''
            }
          ...
          <td style="width:30%; text-align:right; vertical-align:top; padding:0;">
            ${
              (item.issued_month || item.issued_year)
                ? `<p style="margin:0; padding:0; color:#000;">${item.issued_month || ''} ${item.issued_year || ''}</p>`
                : ''
            }
          </td>
        </tr>
