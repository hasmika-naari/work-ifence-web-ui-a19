...
export class AppAdminDashboardService {
  ...
  getPlans(params: Record<string, unknown>): Observable<PagedResponse<any>> {
    const preferredParams = this.normalizePlanSortParams(params);
    ...
    return this.getPaged<any>('/api/subscription-plans', preferredParams).pipe(
      catchError((error: { status?: number; error?: { message?: string } }) => {
        ...
        const fallbackNameParams = { ...preferredParams, sort: 'name,asc' };
  ...
  private normalizePlanSortParams(params: Record<string, unknown>): Record<string, unknown> {
    ...
    const allowedFields = new Set(['id', 'planCode', 'name', 'price', 'billingCycle', 'isActive', 'createdDate', 'lastModifiedDate']);
    const safeField = allowedFields.has(field) ? field : 'planCode';
