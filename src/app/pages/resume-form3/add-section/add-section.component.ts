...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, MatCardModule, ButtonModule, MatButtonModule, MatIconModule],
...
export class AddSectionComponent implements OnInit, OnDestroy {
  ...
  @Output() sectionSelected = new EventEmitter<SectionDesc>();
    ...
    constructor(private router: Router, public userStore : UserStoreService, private sanitizer: DomSanitizer) {
      ...
      // });
      // Initialize with all sections to ensure it works
  // Use only the first 2 sections for testing
  this.add_sections = sections?.slice(0, 2);
  console.log('[AddSectionComponent] constructor: initialized with sections:', this.add_sections.length);
    }
    ngOnDestroy(): void {
    ...
    ngOnInit(): void {
      console.log('[AddSectionComponent] ngOnInit started');
      console.log('[AddSectionComponent] ngOnInit: sections already loaded in constructor, count:', this.add_sections.length);
      // No complex logic needed - just ensure we have sections
      if (!this.add_sections || this.add_sections.length === 0) {
        this.add_sections = sections?.slice(0, 2);
        console.log('[AddSectionComponent] ngOnInit: Fallback loaded sections:', this.add_sections.length);
      }
    }
  ...
  getSectionSvgIllustration(sectionType: string): SafeHtml {
    try {
