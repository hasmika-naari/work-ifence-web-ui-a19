...
import { Education, JobDescriptionAIResponse, ProfileSummary, Resume, TemplateVariables } from 'src/app/services/resume.model';
...
export class JobDescriptionComponent implements OnInit, OnDestroy {
  ...
  setSummaryValues(){
    // this.summaryForm.controls['profile_summary'].setValue(this.resumeSignalForm().profileSummary.profile_summary);
    // this.summaryForm.controls['position_highlight'].setValue(this.resumeSignalForm().profileSummary.position_highlight);
    // this.summaryForm.controls['skills_highlight'].setValue(this.resumeSignalForm().profileSummary.skills_highlight);
  ...
  saveAndContinue(){
    ...
    summary.profile_summary = this.summaryForm.controls['profile_summary'].value?this.summaryForm.controls['profile_summary'].value : "";
  ...
  optimizeText() : void{
      ...
      const objective_prompt = this.promptService.prompt_testing(this.summaryForm.controls['profile_summary'].value?this.summaryForm.controls['profile_summary'].value : '');
      this.resumeService.requestOpenAI({ "prompt" : objective_prompt}).subscribe((res : any)=>{
        ...
        let content : JobDescriptionAIResponse= JSON.parse(res['choices'][0]['message']['content']);
