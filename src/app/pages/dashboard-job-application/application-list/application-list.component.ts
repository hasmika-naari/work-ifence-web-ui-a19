import { Component, Input, OnChanges, OnDestroy, OnInit, Signal, SimpleChanges, inject } from '@angular/core';
...
import { JobApplication, JobResume, JobResumeRequest, Resume } from 'src/app/services/resume.model';
...
export class ApplicationListComponent implements OnInit, OnChanges, OnDestroy {
  ...
  private appUtilService: AppUtilService = inject(AppUtilService);
    ...
    jobApplications:  Signal<Array<JobApplication>> = this.userStore.getJobApplicationsList();
    filteredApps: Signal<Array<JobApplication>> = this.userStore.getFilteredJobApplicationsList();
    userAccount: Signal<Account> = this.userStore.getUserAccount();
    selectedJobAppLication : Signal<JobApplication> = this.userStore.getSelectedJobApplication();
    ...
    ngOnInit() {
        this.subs.push(this.wifenceDataService.getJobApplications(
            this.userAccount().id).subscribe((data: Array<JobApplication>) => {
            
            // this.applications = [...data];
            // const updatedResumes = this.resumes.map(resume => ({
            //     ...resume,
            //     imageBytes : this.handleResumeListitems(resume.userName, resume.fileName)
            //   }));
            // this.filteredResumes = [...this.resumes];
              this.userStore.addJobApplicationList(data);
            }));
    ...
    ngOnChanges(changes: SimpleChanges): void {
        ...
        if(this.filterText !== null){
            // this.filteredResumes = [...this.resumes.filter((r) => r.title?r.title.includes(this.filterText):'')];
        }else{
            // this.filteredResumes = [...this.resumes];
        }
    }
    ...
    addToWishList($event: any, resume: JobApplication){
    ...
    menuActionHandler($event: any, item: JobApplication, option: any){
        if(option.label === 'Edit'){
            
            if(item && item.job_application_details.id) {
                this.userStore.setJobApplication(item);
        ...
        //     let custom_fileName = 'copyOf' + item.title.replace(/\s+/g, "") + this.appUtilService.generateUniqueString() + '.pdf';
        ...
        //     request.status = item.status?"ACTIVE":"IN_ACTIVE";
        ...
        //     this.subs.push(this.resumeService.saveResume(request).subscribe(() => {
        //         this.resumeService.getResumeListByOwnerId(this.userAccount().id).subscribe(data => {
    ...
    confirmDeleteDialog(item : JobApplication): void {
        ...
        dialogRef.afterClosed().subscribe(result => {
          if(result.event === "CONFIRM"){
            this.isActionInProgress = true;
            if(item.job_application_details.id){
                this.subs.push(this.ifenceService.deleteAppliedJobById(item.job_application_details.id).subscribe(() => {
                    
                    this.ifenceService.getAppliedJobsByUserId(this.userAccount().id).subscribe(data => {
    ...
    update($event: any, field_name : string, resume : JobApplication){
    ...
    // showTemplate($event, ){
    //     this.router.navigate(['/job-applications/application'])    
    // }
